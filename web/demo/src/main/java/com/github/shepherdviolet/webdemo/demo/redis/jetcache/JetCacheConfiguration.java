...
import sviolet.slate.common.helper.jetcache.lettuce.SyncRedisLettuceCacheBuilder;
...
public class JetCacheConfiguration {
     ...
     * redis://host1:7000,redis://host2:7000,redis://host3:7000
     * redis://password@host1:7000,redis://password@host2:7000,redis://password@host3:7000
     */
...
//                    .validateClusterNodeMembership(false)//直接关闭集群节点检查, 默认true(避免Redis集群拓扑变化时报出错误: Connection to ?:? not allowed. This connection point is not known in the cluster view)
    ...
    private List<RedisURI> parseUri(String uri){
        if (uri == null || uri.length() <= 0) {
            throw new RuntimeException("property '" + PROPERTY_PREFIX + "uri' is required");
        }
        ...
        for (String item : array) {
            ...
            if (item.length() > 0) {
                uriList.add(item);
                logger.info("JetCache URI: " + item);
            }
        }
        if (uriList.size() <= 0) {
            throw new RuntimeException("property '" + PROPERTY_PREFIX + "uri' is invalid, no valid uri contains");
        }
...
//                .redisClient(lettuceRedisClient);
        //同步操作(牺牲性能, 防误用), 这个类在slate-common库中, 也可以参考下文
