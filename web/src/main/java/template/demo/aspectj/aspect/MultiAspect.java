...
public class MultiAspect {
    ...
    /**
     * 切点
     * template.demo.aspectj.service任意子包下任意类, 两个方法参数, 且均标注了@Mark注解
     */
    @Pointcut(value = "execution(* template.demo.aspectj.service..*.*(@template.demo.aspectj.aspect.Mark (*), @template.demo.aspectj.aspect.Mark (*)))")
     ...
     * 匿名切点
     * args切点开销很大, 慎用
     * template.demo.aspectj.service任意子包下任意类, 方法有@Mark注解, 实际传入的参数为Integer, 注意这个类型是实际传入参数的类型, 不是方法签名
     */
    @Before(value = "execution(@template.demo.aspectj.aspect.Mark * template.demo.aspectj.service..*.*(..)) && args(param1)", argNames = "param1")
